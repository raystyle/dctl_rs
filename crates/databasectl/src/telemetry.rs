...
//! `dctl telemetry enable`, which is itself consent (stronger than
...
//! omission rules.
//!
//! Transport is a detached child process (`dctl telemetry send`,
...
/// Production ingest endpoint. Upstream shipped a Cloudflare worker in front
/// of ClickHouse Cloud; this fork sends nowhere by default — a collector you
/// own can be set through [`URL_ENV`] (self-hosted ingest or dev).
const DEFAULT_ENDPOINT: &str = "";

/// Overrides the ingest endpoint (integration tests, local worker dev).
const URL_ENV: &str = "DCTL_TELEMETRY_URL";
/// Carries the serialized payload from the parent to the hidden send child.
const PAYLOAD_ENV: &str = "DCTL_TELEMETRY_PAYLOAD";
/// When truthy: print the exact payload to stderr and send nothing.
const DEBUG_ENV: &str = "DCTL_TELEMETRY_DEBUG";
...
/// `dctl update` atomically replaces the binary on disk before the
/// send child is spawned. On Linux, `/proc/self/exe` then resolves to
/// `.../dctl (deleted)`, the spawn fails, and the event for the
...
/// `~/.dctl/telemetry.json`. `None` when the home directory cannot be
...
/// argument list — including everything after a `--` — as a dctl
...
fn print_first_run_notice() {
    ...
    let _ = writeln!(
        std::io::stderr(),
        "\nNote: dctl collects anonymous usage data to help improve the CLI:\n\
         ...
         Opt out: `dctl telemetry disable` or DO_NOT_TRACK=1.\n\
         Details: {DOCS_URL}"
    );
}
...
/// Re-invoke this binary as `dctl telemetry send` with the payload
...
pub async fn run_child_send() {
    ...
    if url.is_empty() {
        // No collector configured: a fork build must stay silent rather than
        // report to the upstream endpoint.
        return;
    }
...
fn print_status() {
    ...
    match load_state_from(&path) {
        ...
        State::Enabled => {
            println!(
                "Telemetry is enabled. Disable with `dctl telemetry disable` or DO_NOT_TRACK=1.\nDetails: {DOCS_URL}"
            );
        }
    }
}
...
mod tests {
    ...
    fn debug_env_prints_instead_of_sending() {
        ...
        let path = dir.path().join("telemetry.json");
        save_state_to(&path, false).unwrap();
        let env = env_of(&[("DCTL_TELEMETRY_DEBUG", "1")]);
        assert!(matches!(
    ...
    fn primary_json_file_aliases_capture_only_canonical_argument_names() {
        for alias in ["config", "config-file"] {
            ...
            let mut args = vec!["dctl", "local", "server", "start"];
    ...
    fn capture_reports_names_only_never_values_or_positionals() {
        ...
        // lands in the path.
        for position in 2..=5 {
            let mut args = vec!["dctl", "local", "server", "start", "SECRET-NAME"];
    ...
    fn capture_distinguishes_bare_named_and_flag_named_stop() {
        let bare = capture_from(&["dctl", "local", "server", "stop"]);
        ...
        let named = capture_from(&["dctl", "local", "server", "stop", "SECRET-NAME"]);
        ...
        let flagged = capture_from(&["dctl", "local", "server", "stop", "--name", "SECRET-NAME"]);
    ...
    fn lossy_bare_invocation_is_missing_subcommand() {
        let inv = capture_lossy_from(&["dctl"]);
    ...
    fn lossy_root_help_records_the_help_flag() {
        let inv = capture_lossy_from(&["dctl", "--help"]);
        assert_eq!(inv.command, "");
    ...
    fn lossy_nested_help_keeps_the_command_path() {
        let inv = capture_lossy_from(&["dctl", "local", "server", "--help"]);
    ...
    fn lossy_short_help_and_version_record_long_names() {
        let inv = capture_lossy_from(&["dctl", "local", "-h"]);
        assert_eq!(inv.command, "local");
        assert_eq!(inv.flags, ["help"]);
        assert_eq!(inv.outcome, "help");

        let inv = capture_lossy_from(&["dctl", "-V"]);
        assert_eq!(inv.command, "");
    ...
    fn lossy_typoed_subcommand_stops_and_carries_the_suggestion() {
        let inv = capture_lossy_from(&["dctl", "local", "servr", "start"]);
    ...
    fn lossy_unknown_flag_stops_the_walk() {
        let inv = capture_lossy_from(&["dctl", "local", "--frobnicate", "list"]);
        assert_eq!(inv.command, "local");
        assert!(inv.flags.is_empty());
        assert_eq!(inv.outcome, "unknown_argument");
        let json = serde_json::to_string(&build_payload(&inv, 2, &env_of(&[]), None)).unwrap();
    ...
    fn lossy_double_dash_stops_the_walk() {
        let inv = capture_lossy_from(&["dctl", "local", "--", "SECRET-POSITIONAL"]);
        assert_eq!(inv.command, "local");
        assert!(inv.flags.is_empty());
        let json = serde_json::to_string(&build_payload(&inv, 2, &env_of(&[]), None)).unwrap();
    ...
    fn hostile_positional_fixtures_never_reach_the_payload() {
        ...
        for hostile in HOSTILE {
            // Successful parse.
            let inv = capture_from(&["dctl", "local", "server", "stop", hostile]);
            assert_eq!(inv.positionals, ["name"]);
            let json = serde_json::to_string(&build_payload(&inv, 0, &env_of(&[]), None)).unwrap();
            ...
            let inv =
                capture_lossy_from(&["dctl", "local", "server", "stop", hostile, "--frobnicate"]);
            assert_eq!(inv.positionals, ["name"]);
            let json = serde_json::to_string(&build_payload(&inv, 2, &env_of(&[]), None)).unwrap();
            assert!(!json.contains(hostile), "leaked {hostile}: {json}");
    ...
    fn state_path_is_telemetry_json_under_base_dir() {
        ...
        assert!(path.ends_with(".dctl/telemetry.json"));
    }
}
