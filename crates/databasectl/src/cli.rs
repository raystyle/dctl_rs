...
mod tests {
    ...
    fn whole_command_tree_has_descriptions() {
        ...
        visit_commands(&Cli::command(), "dctl", &mut |command, path| {
            if command
                .get_about()
    ...
    fn built_help_tree_keeps_shared_options_in_a_final_ordered_block() {
        ...
        tree.build();
        visit_commands(&tree, "dctl", &mut |command, path| {
    ...
    fn built_help_tree_preserves_inherited_flags() {
        ...
        visit_commands(&tree, "dctl", &mut |command, path| {
            ...
            let required: &[&str] = if path.starts_with("dctl local") {
                &["json"]
            } else {
                &[]
            };
    ...
    fn version_flags_preserve_the_version_action() {
        for flag in ["-V", "--version"] {
            let error = Cli::try_parse_from(["dctl", flag])
                .err()
    ...
    fn unknown_command_exits_with_a_usage_error() {
        assert_eq!(
            Cli::try_parse_from(["dctl", "unknown-command"])
                .err()
    ...
    fn skills_requires_selection_only_without_a_terminal() {
        for flags in [
            ...
            let cli =
                Cli::try_parse_from(["dctl", "skills"].into_iter().chain(flags.iter().copied()))
                    .unwrap();
    ...
    fn parses_skills_all_and_agent_flags() {
        let cli = Cli::try_parse_from(["dctl", "skills", "--all"]).unwrap();
        let Commands::Skills(args) = cli.command else {
        ...
        let cli = Cli::try_parse_from(["dctl", "skills", "--global"]).unwrap();
        let Commands::Skills(args) = cli.command else {
            panic!("expected skills command");
        };
        assert!(args.global);
        ...
        let cli = Cli::try_parse_from(["dctl", "skills", "--detected-only"]).unwrap();
        let Commands::Skills(args) = cli.command else {
            panic!("expected skills command");
        };
        assert!(args.detected_only);
    ...
    fn skills_agent_accepts_every_supported_agent_and_rejects_unknown_values() {
        ...
        let cli = Cli::try_parse_from(["dctl", "skills", "--agent", &joined]).unwrap();
        let Commands::Skills(args) = cli.command else {
            panic!("expected skills command");
        };
        assert_eq!(args.agents, supported);

        let error = Cli::try_parse_from(["dctl", "skills", "--agent", "unknown"])
            .err()
