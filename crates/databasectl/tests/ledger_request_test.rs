...
fn garbage_key_fails_before_any_request() {
    for bad in ["not-a-key", "zzzz", "sha256:123"] {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(
            stderr.contains("neither a PEM block, a 64-hex seed, nor a readable path"),
            "{bad}: {stderr}"
        );
    }
}
...
fn reads_do_not_require_the_private_key() {
    // Reads are unsigned GETs; demanding the key would lock read-only users
    // out. An unroutable proxy pins the failure to the network layer —
    // proving the run got past key resolution without ever loading a key.
    let output = dctl()
        .env("HOME", home_without_key())
        .env("https_proxy", "http://127.0.0.1:9")
    ...
    assert!(
        stderr.contains("ledger request failed"),
        "the failure must be the request, never the key: {stderr}"
    );
    assert!(
...
#[test]
fn env_key_may_carry_a_path_to_the_pem() {
    // The vault/CI injection form: DCTL_LEDGER_KEY points at a file. The
    // seed parses from that file, so the failure is the (unroutable) request.
    let home = home_without_key();
    let seed = tempfile::tempdir().unwrap();
    let key_path = seed.path().join("ledger.pem");
    std::fs::write(
        &key_path,
        "9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60",
    )
    ...
    let output = dctl()
        .env("HOME", home)
        .env("DCTL_LEDGER_KEY", key_path.display().to_string())
        .env("https_proxy", "http://127.0.0.1:9")
        .args([
            "ledger",
            "issue",
    ...
    assert!(
        stderr.contains("ledger request failed"),
        "the path form must resolve the key: {stderr}"
    );
}
