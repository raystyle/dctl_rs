...
- [ ] `server start`(即 `dctl clickhouse start`)新增 `--bind <IP>`:默认 loopback 发布之外追加一个发布面;`0.0.0.0` 为全接口特殊值,替换 loopback 而非叠加(同口通配与定点绑定冲突);face 为 127.0.0.1 时去重为单面
- [ ] 不带 `--bind` 行为零变化(仅 loopback,默认安全面不动)
- [ ] 值必须是可解析 IP 地址(v4/v6 皆收,规范化存储),clap 层拒收非 IP
- [ ] 端口可用性检查与自动挑选覆盖绑定面:显式口在任一面被占即报 PortInUse,自动挑选避开任一面被占的口
- [ ] resume 语义与端口旗标一致:`--bind` 在 resume 时忽略(容器保持既有绑定)
- [ ] 帮助文本、README、CONTEXT FOR AGENTS 注记同步
- [ ] 测试:clap 解析正反例、面感知端口检查单测、假 Docker create 体的 HostIp 断言
