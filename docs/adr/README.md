# ADR 索引

> 架构决策记录索引;新建拷 `docs/adr/ADR-0000-template.md`,编号四位接当前最大号,退役不复用。状态机:proposed 到 accepted 到 superseded,只进不退。

| id | 状态 | 标题 | 替代 |
| --- | --- | --- | --- |
| ADR-0001 | accepted | fork clickhousectl 并整体剪除 cloud 栈 | 无 |
| ADR-0002 | accepted | 命名与状态目录(dctl/databasectl/.dctl) | 无 |
| ADR-0003 | accepted | telemetry 默认编译排除且上报端点置空 | 无 |
| ADR-0004 | accepted | 分发只走 GitHub Releases | 无 |
| ADR-0005 | accepted | 接入 FalkorDB 图数据库引擎,Docker 托管全功能镜像 | 无 |
| ADR-0006 | accepted | 集成 ledger 公共账本,顶层 ledger 命令域与本地密档签名道 | 无 |
| ADR-0007 | accepted | ClickHouse 切 Docker 容器管理,三引擎统一,client 在 dctl 内集成 | 无 |
| ADR-0008 | accepted | 私仓直连与镜像拉取回落序(registry.ohmygh.com,OCI layout) | 无 |
| ADR-0009 | accepted | Postgres 与 FalkorDB 原生客户端集成(tokio-postgres 加 falkordb,程序化道原生) | 无 |
| ADR-0010 | accepted | 镜像拉取链序反转为私仓优先,支持 --registry 按次自定义源 | ADR-0008 决策 1(部分) |
| ADR-0011 | accepted | 免密密钥身份认证:本地 CA 与三引擎 mTLS 架构 | 无 |
